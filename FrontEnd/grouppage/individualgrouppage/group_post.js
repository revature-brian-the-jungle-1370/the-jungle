...
async function createGroupPost() {
  ...
  console.log("localStorage.getItem(\"user_id\"): " + Number(localStorage.getItem("user_id")))
  console.log("localStorage.getItem(\"group_id\"): " + Number(localStorage.getItem("group_id")))
...
async function populateData(responseBody) {
  ...
  for (let post of responseBody) {
    let date_1 = post.date_time_of_creation.slice(0, -7);
    let postBox = document.createElement('div');
    postBox.innerHTML = `
    <p style=" float:right;font-size: 1em;color:grey;"> ` + date_1 + `</p>
