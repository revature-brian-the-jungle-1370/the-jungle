...
async function updateUserProfileData(){
    ...
    let url = "http://localhost:5500/user/profile/update/"+loggedInUserId;
...
async function getProfileUser(userId,key){
    let response = await fetch("http://localhost:5500/user/"+userId);
    if (response.status === 200) {
      let body = await response.json();
...
async function getUserFollowers(){
    let url = "http://localhost:5500/user/followers/"+userId
...
async function getFollowerImage(followerBody){
    for(follower in followerBody){
        ...
        let url = `http://localhost:5500/user/image/${followerBody[follower]}`;
        // console.log(url);
        let response = await fetch(url);
        if(response.status === 200){
...
async function getGroupsForUser(){
    let url = "http://localhost:5500/group/user/"+userId
...
async function follow_user(){
    ...
    let followResponse = await fetch("http://localhost:5500/user/" + loggedInUserId + "/followed/" + userId, {
        method: "POST",
        mode: "cors",
        headers: {"Content-Type": "application/json"},
        body:followJson
